...
makecontext(ucontext_t *ucp, void (*func)(void), int argc, ...)
{
   ...
   if(argc > 1)
   {
      printf( "crash: makecontext(): wrong number of arguments argc==%d\n", argc );
      //exit(-1);
      //*(int*)0 = 0; ///kevin: are they trying for a breakpoint?
      assert( argc <= 2 );
      static int* bok = (int*)0;
      *bok = 0;
   }
   if (argc != 1)
   {
      printf( "warning: makecontext(): wrong number of arguments argc==%d\n", argc );
   }
	va_start(va, argc);
